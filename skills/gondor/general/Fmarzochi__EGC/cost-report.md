---
description: Generate a local Gemini CLI cost report from a cost-tracker SQLite database.
argument-hint: [csv]
---
...
is already writing usage rows to `~/.Gemini-cost-tracker/usage.db`.
...
trusted Gemini CLI cost-tracking hook/plugin first.
...
```bash
sqlite3 -csv -header ~/.Gemini-cost-tracker/usage.db "
